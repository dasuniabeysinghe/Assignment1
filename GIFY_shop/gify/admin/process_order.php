...
if (empty($cart) || !$addressId || !$paymentMethod) {
...
try {
    ...
    $deliveryAddress = $address['building_name'] . ', ' . $address['faculty'] . ' - ' . $address['address_line'];
    ...
    // Create order
    $stmt = $pdo->prepare("INSERT INTO orders (user_id, order_number, total_amount, payment_method, delivery_address) VALUES (?, ?, ?, ?, ?)");
    $stmt->execute([$userId, $orderNumber, $totalAmount, $paymentMethod, $deliveryAddress]);
